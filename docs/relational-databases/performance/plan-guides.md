...
```  
Sehen Sie sich als weiteres Beispiel die folgende SQL-Anweisung an, die mit [sp_executesql](../../relational-databases/system-stored-procedures/sp-executesql-transact-sql.md) übermittelt wird.

```sql  
exec sp_executesql N'SELECT * FROM Sales.SalesOrderHeader
where SalesOrderID =  @so_id', N'@so_id int', @so_id = 43662;  
```  
 Erstellen Sie die folgende Planhinweisliste, und verwenden Sie den Abfragehinweis `OPTION (RECOMPILE)` im Parameter `@hints`, um einen eindeutigen Plan für jede Ausführung der Abfrage zu erstellen. 
